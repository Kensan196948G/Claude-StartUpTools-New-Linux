...
- **実行グループ限定（`projectGroups`）**: `config/config.json` の `projectGroups` にグループフォルダ名を
  ...
  並び順がそのまま L 番号になる。S1（バックグラウンド）は従来どおり「①グループ選択 → ②配下から選択」の
  2 段階 UI。`autoInitProjects` もグループ配下のみを対象にする（グループフォルダを毎回 git init して
  単独 repo 化する退行を防止）。空配列（既定）なら projects 直下全体が対象で、L1 単独のフラット選択。
