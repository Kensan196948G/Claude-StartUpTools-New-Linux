...
- 5 時間上限、Loop Guard、STABLE 判定、再開容易性を明文化して暴走を防ぐ
...
- Linux cron（月〜土・プロジェクト別スケジュール・300分制限）が自律実行を担う
- `cron-launcher.sh` が各プロジェクトで Claude を 5 時間セッション起動
- `New-CronSchedule.ps1` でスケジュール登録・管理（メニュー 14）
- 5 時間上限で安全停止（timeout → finalize → メール通知）
- Agent Teams 会話の可視化
- README とドキュメントの継続更新
