...
team__pane_cmd() {
  ...
  if ccsu_claude_supports_name "$CLAUDE_BIN"; then
    name_args="--name '$(CCSU_SESSION_ROLE="$role" ccsu_claude_session_name "$project")' "
  fi
  printf "CLAUDE_PROJECT='%s' CLAUDEOS_HOOKS_DIR='%s/.claude/claudeos/scripts/hooks' timeout --foreground %ss %s %s%s" \
    "$project" "$workdir" "$dur_sec" "$CLAUDE_BIN" "${model_args:+$model_args }" "$name_args"
}
